...
const Cart = {
    ...
    addToCart(button) {
        ...
        // Check if this is a premium collection box and enforce max quantity of 3
        ...
        if (existingItem) {
            if (isPremiumCollection && (existingItem.quantity + quantity) > 3) {
                const maxCanAdd = 3 - existingItem.quantity;
                if (maxCanAdd <= 0) {
                    utils.showNotification('Maximum 3 premium collection boxes allowed per customer', 'error');
                    return;
                } else {
                    existingItem.quantity = 3; // Set to max
                    utils.showNotification(`Added ${maxCanAdd} more (Maximum 3 premium collections reached)`, 'info');
                }
            } else {
                existingItem.quantity += quantity;
            }
        } else {
            if (isPremiumCollection && quantity > 3) {
                product.quantity = 3; // Limit initial quantity to 3
                state.cart.push({...product, productId, quantity: 3});
                utils.showNotification('Limited to maximum 3 premium collection boxes', 'info');
            } else {
                ...
                <div class="cart-modal-body">
                    ...
                    <div class="free-shipping-section">
                        ...
                        <div class="free-shipping-info">
                            ...
                            <div class="shipping-amount">≥ $120.00</div>
                        </div>
                        <div class="shipping-progress">
                            <div class="progress-bar">
                                <div class="progress-fill" id="shipping-progress-fill"></div>
                            </div>
                            <div class="shipping-text" id="shipping-text">Spend $120.00 more to get Free Shipping!</div>
                        </div>
                    </div>
    ...
    updateCartModal() {
        ...
        state.cart.forEach((item, index) => {
            ...
            // Disable + button if at max quantity for premium collections
            const plusDisabled = isPremiumCollection && item.quantity >= 3 ? 'disabled' : '';
            const maxText = isPremiumCollection ? '<span class="max-text">(Max: 3)</span>' : '';
        ...
        const shippingCost = discountedSubtotal >= 120 ? 0 : 15;
    ...
    updateQuantity(index, change) {
        if (state.cart[index]) {
            ...
            } else if (isPremiumCollection && newQuantity > 3) {
                utils.showNotification('Maximum 3 premium collection boxes allowed per customer', 'error');
                return; // Don't update quantity beyond 3
            } else {
...
function nextSlide() {
    ...
    goToSlide(nextIndex);
    // Synchronize announcement banner transition
...
function startSlideshow() {
    ...
    slideInterval = setInterval(nextSlide, 5000); // 5 second interval
}
