...
const Navigation = {
    ...
    setupMobileMenu() {
        const mobileBtn = document.querySelector('.hamburger-menu-btn, #hamburger-menu-btn, .mobile-menu-btn');
        const navMenu = document.querySelector('.nav-menu');

        console.log('Mobile menu setup - Button found:', mobileBtn, 'Nav menu found:', navMenu);
        ...
        if (mobileBtn && navMenu) {
            ...
            // Setup dropdown functionality in mobile menu
            const dropdownItems = navMenu.querySelectorAll('.has-dropdown');
            dropdownItems.forEach(item => {
                const trigger = item.querySelector('.dropdown-trigger, .nav-link');
                if (trigger) {
...
const Wishlist = {
    ...
    loadWishlistFromStorage() {
        const savedWishlist = localStorage.getItem('rippoke_wishlist');
        if (savedWishlist) {
            try {
    ...
    extractWishlistData(productCard, button) {
        ...
        return {
            ...
            title: titleElement ? titleElement.textContent : 'Unknown Product',
            price: priceElement ? utils.parsePrice(priceElement.textContent) : 0,
            image: imageElement ? imageElement.src : '/api/placeholder/150/200',
    ...
    addToWishlist(item) {
        ...
        if (!existingItem) {
            ...
            localStorage.setItem('rippoke_wishlist', JSON.stringify(state.wishlist));
            this.updateWishlistDisplay();
        }
    },
...
const Cart = {
    ...
    setupAddToCartButtons() {
        ...
        document.addEventListener('touchend', handleCartButtonClick);
        
        // Also add direct listeners to new arrivals buttons for extra reliability
        const setupNewArrivalsButtons = () => {
            const newArrivalsButtons = document.querySelectorAll('.new-arrivals-section .btn-add-cart');
            newArrivalsButtons.forEach(button => {
                // Remove any existing listeners
                button.removeEventListener('click', this.handleDirectCartClick);
                button.removeEventListener('touchend', this.handleDirectCartClick);
                
                // Add fresh listeners
                button.addEventListener('click', this.handleDirectCartClick.bind(this));
                button.addEventListener('touchend', this.handleDirectCartClick.bind(this));
            });
        };
        
        // Set up buttons immediately and after DOM changes
        setupNewArrivalsButtons();
        setTimeout(setupNewArrivalsButtons, 1000);
    },
    
    handleDirectCartClick(e) {
        e.preventDefault();
        e.stopPropagation();
        console.log('Direct cart button clicked (mobile optimized):', e.target);
...
const HamburgerMenu = {
    init() {
        ...
        console.log('HamburgerMenu init - Button:', this.hamburgerBtn, 'Menu:', this.navMenu);
        
        if (this.hamburgerBtn && this.navMenu) {
            this.bindEvents();
            console.log('HamburgerMenu events bound successfully');
        } else {
            console.error('HamburgerMenu: Could not find required elements');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Final mobile menu failsafe - ensure it works
    setTimeout(() => {
        const mobileBtn = document.querySelector('.hamburger-menu-btn, #hamburger-menu-btn, .mobile-menu-btn');
        const navMenu = document.querySelector('.nav-menu, #nav-menu');
        
        console.log('Final mobile menu check:', mobileBtn, navMenu);
        
        if (mobileBtn && navMenu && window.innerWidth <= 1024) {
            ...
            newBtn.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                console.log('Mobile menu button clicked - final failsafe');
                navMenu.classList.toggle('mobile-open');
                newBtn.classList.toggle('active');
            ...
            console.log('Mobile menu failsafe applied successfully');
        }
    }, 500);
