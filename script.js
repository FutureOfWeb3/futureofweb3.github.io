...
const utils = {
    ...
    // Parse price from string (handles multiple currency symbols)
...
const Cart = {
    ...
    updateCartModal() {
            ...
            html += `
                <div class="cart-item">
                    ...
                    <div class="cart-item-details">
                        ...
                        <div class="cart-item-price">${utils.formatCurrency(item.price, state.currency.current)} each</div>
                    </div>
        ...
        subtotalElement.textContent = utils.formatCurrency(total, state.currency.current);
        totalElement.textContent = utils.formatCurrency(total, state.currency.current);
    },
...
const CurrencyConverter = {
    ...
    bindEvents() {
        // Add event listeners to all currency selectors across all pages
    ...
    loadSavedCurrency() {
        const savedCurrency = localStorage.getItem('rippoke_currency') || 'USD';
        state.currency.current = savedCurrency;
        
        // Update all currency selectors to show the saved currency
        const currencySelectors = document.querySelectorAll('#currency, select[name="currency"]');
        currencySelectors.forEach(selector => {
            selector.value = savedCurrency;
        });
        
        // Update prices on page load if not USD
    ...
    async updateRatesIfNeeded() {
        // Check if rates were updated in the last hour
        const lastUpdate = localStorage.getItem('rippoke_rates_updated');
        ...
        if (!lastUpdate || (Date.now() - parseInt(lastUpdate)) > oneHour) {
            ...
            // Load saved rates
            const savedRates = localStorage.getItem('rippoke_exchange_rates');
            if (savedRates) {
                state.currency.rates = JSON.parse(savedRates);
            }
        }
    },
    ...
    async fetchLatestRates() {
        ...
        // You can replace this with any other API or use static rates
        try {
            const response = await fetch('https://api.exchangerate-api.com/v4/latest/USD');
            const data = await response.json();
            
            // Update our supported currencies
            ...
            localStorage.setItem('rippoke_exchange_rates', JSON.stringify(state.currency.rates));
            localStorage.setItem('rippoke_rates_updated', Date.now().toString());
            
            console.log('Exchange rates updated:', state.currency.rates);
        } catch (error) {
    ...
    changeCurrency(newCurrency) {
        const oldCurrency = state.currency.current;
        state.currency.current = newCurrency;
        
        // Save the new currency preference
        localStorage.setItem('rippoke_currency', newCurrency);
        
        // Update all currency selectors across the page
        const currencySelectors = document.querySelectorAll('#currency, select[name="currency"]');
        currencySelectors.forEach(selector => {
        ...
        this.updateAllPrices(newCurrency, oldCurrency);
        ...
        utils.showNotification(`Currency changed to ${currencyNames[newCurrency]}`, 'success');
        
        // Debug log for testing
        console.log(`Currency conversion: ${oldCurrency} → ${newCurrency}`, state.currency.rates);
    },
    ...
    convertPrice(amount, fromCurrency = 'USD', toCurrency = 'USD') {
        ...
        if (fromCurrency !== 'USD') {
            usdAmount = amount / state.currency.rates[fromCurrency];
        }
        
        // Convert from USD to target currency
        if (toCurrency !== 'USD') {
            return usdAmount * state.currency.rates[toCurrency];
        }
        
        return usdAmount;
    },

    updateAllPrices(newCurrency, oldCurrency = 'USD') {
        // Update all price elements on the page
        const priceElements = document.querySelectorAll('.current-price, .original-price, .bundle-price, .price');
        ...
        priceElements.forEach(element => {
            ...
            if (priceMatch) {
                const currentAmount = parseFloat(priceMatch[0].replace(/,/g, ''));
                const convertedAmount = this.convertPrice(currentAmount, oldCurrency, newCurrency);
                
                // Format the new price
                const formattedPrice = utils.formatCurrency(convertedAmount, newCurrency);
                element.textContent = formattedPrice;
            }
        });

        // Update data-price attributes in buttons for cart functionality
        const cartButtons = document.querySelectorAll('[data-price]');
        cartButtons.forEach(button => {
            const currentPrice = parseFloat(button.getAttribute('data-price'));
            const convertedPrice = this.convertPrice(currentPrice, oldCurrency, newCurrency);
            button.setAttribute('data-price', convertedPrice.toFixed(2));
        });

        // Update cart items if any exist
        if (state.cart.length > 0) {
            state.cart.forEach(item => {
                item.price = this.convertPrice(item.price, oldCurrency, newCurrency);
            });
            Cart.updateCartDisplay();
        }

        // Update shipping costs in announcements
        this.updateShippingCosts(newCurrency);
    },

    updateShippingCosts(currency) {
        // Update shipping cost in announcement bar
        const shippingCosts = {
            USD: '$15',
            EUR: '€13',
            GBP: '£11'
        ...
        const shippingAnnouncement = state.announcements.find(ann => 
            ann.text.includes('Shipping')
        );
        
        if (shippingAnnouncement && shippingCosts[currency]) {
            const baseCost = shippingCosts[currency];
            shippingAnnouncement.text = `Tracked Shipping To The US for only ${baseCost}!`;
            
            // Update the announcement if it's currently displayed
            const activeAnnouncement = document.querySelector('.announcement-item.active');
            if (activeAnnouncement && activeAnnouncement.textContent.includes('Shipping')) {
                activeAnnouncement.innerHTML = `
                    <i class="fas fa-shipping-fast"></i>
                    ${shippingAnnouncement.text}
                `;
            }
        }
    },

    // Utility method to get formatted price in current currency
    getFormattedPrice(amount, fromCurrency = 'USD') {
        const convertedAmount = this.convertPrice(amount, fromCurrency, state.currency.current);
        return utils.formatCurrency(convertedAmount, state.currency.current);
    }
};
