...
// Make calculator form fields focusable and improve UX
document.addEventListener('DOMContentLoaded', function() {
    // Auto-focus first field
    const firstField = document.getElementById('calc-year');
    if (firstField) {
        firstField.focus();
    }

    // Add loading state to get quote button
    const getQuoteBtn = document.getElementById('get-quote-btn');
    if (getQuoteBtn) {
        getQuoteBtn.addEventListener('click', function() {
            if (validateCalculatorForm()) {
                ...
                setTimeout(() => {
                    this.textContent = 'Get My Instant Quote';
                    this.disabled = false;
                }, 1500);
            }
        });
