...
document.addEventListener('mousemove', (e) => {
    ...
    // Create trail every few pixels moved
    if (trailCount % 3 === 0) {
        createCloudTrail(mouseX, mouseY);
    }
    trailCount++;
});
...
function createCloudTrail(x, y) {
    ...
    trail.className = 'cloud-trail';
    trail.style.left = (x - 10) + 'px';
    trail.style.top = (y - 10) + 'px';
    
    // Add some randomness to the trail position
    trail.style.left = (x - 10 + Math.random() * 6 - 3) + 'px';
    trail.style.top = (y - 10 + Math.random() * 6 - 3) + 'px';
